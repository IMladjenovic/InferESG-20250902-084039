...
async def test_report_on_file_upload(mocker):
    ...
    file_upload = FileUpload(uploadId="1", filename="test.txt", content="test sustainability document content", contentType="text/plain", size=4)
    ...
    mock_report_agent_instance.invoke.return_value = "# ESG Analysis Report\n\nThis is a generated ESG report."
    ...
    mock_report_agent_instance.invoke.assert_called_once_with("test sustainability document content")
    assert response ==  {"filename": "test.txt", "id": "1", "report": "# ESG Analysis Report\n\nThis is a generated ESG report."}
